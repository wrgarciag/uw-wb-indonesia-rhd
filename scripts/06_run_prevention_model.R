...
# ------------------------------------------------------------------------------
years      <- 2026:2050          # [PAPER] 10-year scale-up window
n_years    <- length(years)
disc_rate  <- 0.03               # [PAPER] 3% discount on costs and benefits

# Simple AU demographic backdrop (denominators for rates only).
pop_2021   <- 288000000             # [LIT] UN WPP 2019, AU ~1.36bn in 2021
pop_growth <- 0.0076              # [LIT] ~2.5%/yr AU population growth
...
p <- list(
  ...
  arf_incident_2021      = 80000,   # [CALIBRATE] new ARF cases/yr; tune to GBD ARF burden
  rhd_incident_2021      = 66000,   # [CALIBRATE] new asymptomatic RHD/yr; tune so that primary
